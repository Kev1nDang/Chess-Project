...
class PawnTest {
    ...
    @Test
    void validMoveDoesNotThrowWhenProbingBackRankWithEmptySquare() {
        // Regression test: querying every square on the board (as the API's legal-move
        // listing does) used to throw ArrayIndexOutOfBoundsException once any piece
        // vacated the far rank, because the double-step check read one row past it
        // before confirming the move was even a same-file double-step.
        Pawn whitePawn = new Pawn("♙", Color.WHITE, "0");
        Board newBoard = new Board(false);
        newBoard.board[6][4] = whitePawn;

        assertFalse(whitePawn.validMove(4, 6, 1, 7, newBoard.board));

        Pawn blackPawn = new Pawn("♟", Color.BLACK, "0");
        Board blackBoard = new Board(false);
        blackBoard.board[1][4] = blackPawn;

        assertFalse(blackPawn.validMove(4, 1, 1, 0, blackBoard.board));
    }
